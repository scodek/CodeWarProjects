...
Event.prototype.subscribe = function(action){
  this.handlerArr.push(action);
}


Event.prototype.unsubscribe = function(action){
  this.handlerArr = this.handlerArr.filter((elem) => (action != elem));
}

Event.prototype.emit = function(...args){
  for(func of this.handlerArr){
    func(...args);
  }
}
